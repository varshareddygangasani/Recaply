...
def send_email(to_emails,subject,body,reply_to=None, name=None):
    email = get_secret("SENDER_EMAIL")
    pwd = get_secret("SENDER_PASSWORD")
...
if not api_key:
    st.warning("⚠️ **API Key Missing**: Please configure your `GROQ_API_KEY` in the Streamlit Cloud **Secrets** dashboard settings (for cloud deployment) or in a `.env` file (for local development).")
...
def gen_summury(transcript):
    ...
    """
    if not client:
        st.error("Groq API client is not initialized. Please set GROQ_API_KEY in Streamlit Secrets or your .env file.")
        return ""
    prompt = f"""
